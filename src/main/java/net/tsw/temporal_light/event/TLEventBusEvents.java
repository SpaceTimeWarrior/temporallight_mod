...
public class TLEventBusEvents {
    ...
    public  static void registerSpawnPlacements(SpawnPlacementRegisterEvent event){
        event.register(TLEntityRegistry.KITSUNE.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Monster::checkMobSpawnRules,SpawnPlacementRegisterEvent.Operation.REPLACE);
        event.register(TLEntityRegistry.KITSUNE.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, Animal::checkAnimalSpawnRules,SpawnPlacementRegisterEvent.Operation.REPLACE);
    }
    @SubscribeEvent
    public static void addCustomTrades(VillagerTradesEvent event) {
        if(event.getType() == TLVillagerRegistry.END_TRADER.get()) {
            ...
            trades.get(5).add((pTrader, pRandom) -> new MerchantOffer(
                    new ItemCost(Blocks.EMERALD_BLOCK,64),
                    new ItemStack(Items.ENDER_DRAGON_SPAWN_EGG,1),
                    1,6,0.5f));
            trades.get(5).add((pTrader, pRandom) -> new MerchantOffer(
                    new ItemCost(Blocks.EMERALD_BLOCK,2),
