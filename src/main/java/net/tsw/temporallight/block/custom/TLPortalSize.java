...
public class TLPortalSize {
    ...
    public static Optional<TLPortalSize> func_242964_a(IWorld world, BlockPos pos, Direction.Axis axis) {
        return func_242965_a(world, pos, (size) -> {
            return size.isValid() && size.portalBlockCount == 0;
        }, axis);
    }

    public static Optional<TLPortalSize> func_242965_a(IWorld world, BlockPos pos, Predicate<TLPortalSize> sizePredicate, Direction.Axis axis) {
        Optional<TLPortalSize> optional = Optional.of(new TLPortalSize(world, pos, axis)).filter(sizePredicate);
        if (optional.isPresent()) {
            return optional;
        } else {
            Direction.Axis direction$axis = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
            return Optional.of(new TLPortalSize(world, pos, direction$axis)).filter(sizePredicate);
        }
    }

    public TLPortalSize(IWorld worldIn, BlockPos pos, Direction.Axis axisIn) {
        this.world = worldIn;
        this.axis = axisIn;
    ...
    private int func_242969_a(BlockPos.Mutable mutablePos) {
        for(int i = 0; i < 21; ++i) {
            ...
            for(int j = 0; j < this.width; ++j) {
                ...
                if (blockstate.matchesBlock(blockRegistry.MAGIWOODPORTAL.get())) {
    ...
    private static boolean canConnect(BlockState state) {
        return state.isAir() || state.isIn(BlockTags.FIRE) || state.matchesBlock(blockRegistry.MAGIWOODPORTAL.get());
    }

    public boolean isValid() {
        return this.bottomLeft != null && this.width >= 2 && this.width <= 256 && this.height >= 2 && this.height <= 256;
    }

    public void placePortalBlocks() {
        BlockState blockstate = blockRegistry.MAGIWOODPORTAL.get().getDefaultState();
